...
function createNotesHTML(notesJSON){
    ...
    for (let index = 0; index < notes.length; index++) {
        ...
        notesHTML += `<li class="notes_item"><div class="edit_notes"><button class="edit-notes-button">Edit</button></div><div class="save_notes"><button class="save-notes-button">Save</button></div><div class="notes-sentence" id="`+notes[index]['jumpToTime']+`">` + notes[index]['sentence'] + `</div></li>`;
    }
...
function addListenersForNotes(){
    ...
    var editNotesButtons = document.getElementsByClassName('edit-notes-button');
    for(var i=0; i< editNotesButtons.length;i++){
        editNotesButtons[i].addEventListener('click',function ( event ) {
            var editNotesButton = event.target;
            var editNotesDiv = editNotesButton.parentElement;
            var saveNotesDiv = editNotesDiv.nextElementSibling;
            var noteDiv = saveNotesDiv.nextElementSibling;
            noteDiv.innerHTML = `<textarea onclick="event.stopPropagation()">${noteDiv.innerText}</textarea>`;
            editNotesDiv.style.display = "none";
            saveNotesDiv.style.display = "block";
            console.log(saveNotesDiv);
        })
    }

    var saveNotesButtons = document.getElementsByClassName('save-notes-button');
    for(var i=0; i< saveNotesButtons.length;i++){
        saveNotesButtons[i].addEventListener('click',function ( event ) {
            console.log(event);
            var saveNotesButton = event.target;
            var saveNotesDiv = saveNotesButton.parentElement;
            var editNotesDiv = saveNotesDiv.previousElementSibling;
            var noteDiv = saveNotesDiv.nextElementSibling;
            noteDiv.innerHTML = `${noteDiv.firstElementChild.value}`;
            saveNotesDiv.style.display = null;
            editNotesDiv.style.display = null;
        })
    }
}
